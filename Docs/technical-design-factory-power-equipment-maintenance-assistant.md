...
3. 支持步骤式检修向导作为主体交互，每一步展示操作说明、安全提醒、依据和预标注图片。
4. 支持预标注图片按步骤切换，不做真实三维建模、自动热点识别或复杂三维交互。
...
| 步骤 | 名称 | 用户动作 | 展示素材 |
|---|---|---|---|
| 1 | 定位异常设备 | 确认发生高温告警的站控柜和工控机 | `step-01-location.png` |
| 2 | 确认安全条件 | 确认通知值班负责人、静电防护和禁止带电拆卸要求 | `step-02-safety.png` |
| 3 | 检查风道 | 查看进风口、出风口和机柜通风是否堵塞 | `step-03-airflow.png` |
| 4 | 检查滤网和风扇 | 检查滤网积尘、风扇异响和运行状态 | `step-04-filter-fan.png` |
| 5 | 恢复验证 | 确认温度告警解除、数据上传稳定、运行状态恢复 | `step-05-verify.png` |
...
1. 由团队提前手动标注图片。
2. 图片放入前端静态资源或后端素材目录。
3. 每个步骤通过 `asset_id` 绑定一张图片。
4. 用户进入某一步时，页面直接展示该图片。
