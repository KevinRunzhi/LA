...
class PresentationStore:
    ...
    def initialize(self, initial_state: dict, case_seed: dict, knowledge_seed: dict):
        ...
        if not exists:
            self.reset(initial_state, case_seed, knowledge_seed)

    def reset(self, state: dict, case_seed: dict, knowledge_seed: dict) -> dict:
        stamp = now_iso()
        ...
        with self.connect() as db:
            for table in ("presentation_state", "cases", "expert_reviews", "knowledge_items", "knowledge_versions", "graph_relations", "engineer_sync_records"):
    ...
    def publish(self, state: dict, case_id: str, knowledge_id: str, draft: dict) -> dict:
        stamp = now_iso()
        version = "1.1"
        ...
        with self.connect() as db:
            ...
            for relation in draft["relations"]:
                db.execute("INSERT INTO graph_relations (knowledge_id,version,source,relation,target,source_case_id,published_by,published_at) VALUES (?,?,?,?,?,?,?,?)", (knowledge_id, version, relation["source"], relation["relation"], relation["target"], case_id, "专家账号", stamp))
